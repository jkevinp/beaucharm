<?php
    ...
    if(isset($_POST['email']) && isset($_POST['password']))
    {
        ...
        $result = mysqli_query($con,"SELECT count(*) FROM `employee` where Username='".$email."' and Password='".$password."'");
        $row = mysqli_fetch_row($result);
        if($row[0] > 0) 
        {
            $user_info = mysqli_query($con, "SELECT * from `employee` where Username='".$email."'");
